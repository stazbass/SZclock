...
public class ClockView {
    ...
    private static String NEW_MINUTE_TEXTURE = "GoldenDot8.png";
    ...
    public List<RenderItem> getMinutes(TimeRecord record) {
        ...
        result.add(new RenderItem(newMinutePosition, new Vector2(newMinuteScale, newMinuteScale), NEW_MINUTE_TEXTURE));
        return result;
    }
    ...
    public List<RenderItem> getMilliseconds(TimeRecord record) {
        ...
        Vector2 scale = new Vector2(scaleVal, scaleVal);
        result.add(new RenderItem(position, scale, MILLISECONDS_TEXTURE));
        if (secondsNow == 59) {
            result.stream().forEach(i -> i.getPosition().interpolate(origin, record.getMilliseconds() % 1000 / 1000.0f, Interpolation.exp5));
        }
        return result;
